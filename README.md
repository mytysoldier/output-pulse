...
現在は本番Secretsと外部サービスの設定前のため、定期同期を停止しています。GitHub Actionsから手動で差分同期を実行してください。本番運用を開始する際は、JST 08:00、14:00、22:00の定期同期を復元します。定刻実行は遅延する可能性があるため、再開後はSlack DMで成功通知が継続していることを確認してください。
