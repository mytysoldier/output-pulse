...
同期WorkflowはJST 08:00、14:00、22:00に実行される。schedule復元後はActions履歴とSlack DMで少なくとも1回の成功を確認し、`app.sync_runs`で重複保存がないことを確認する。cronは遅延する可能性があるため、定刻どおりに開始されない場合でも、直近の実行履歴を基準に確認する。
