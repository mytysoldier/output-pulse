...
- PRの現在状態は`app.pull_requests`へ保存する
- 作成・マージ実績は`app.pull_request_events`へ独立イベントとして保存する
- 期間指定時は、作成日時とマージ日時をそれぞれ対象期間で判定する
- 期間外作成・期間内マージの場合はマージイベントだけを追加する
- 期間内作成・期間外マージの場合は作成イベントだけを追加する
- `(github_node_id, event_type)`の複合キーにより、差分・期間指定・全再同期で重複計上しない
...
通知先はSlack AppのBot Tokenと個人のSlack User IDで決める。実行URLは`https://github.com/<owner>/<repo>/actions/runs/<run_id>`形式だけを通知本文へ含め、リポジトリ名、Token、接続文字列、Slack APIエラー詳細は本文とDBへ保存しない。
